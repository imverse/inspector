import {ReplicationClient} from '@imverse/replication-client-js';


import {calculateEntityHash} from '@imverse/replication-client-js';

import {ProtocolDefinition} from './mech.js';
...
const host = 'wss://inspector.imverse.net:32001';
const log = new Log();
...
class App {
  constructor(elmApp) {
    ...
    this.connection = new ReplicationClient(
        host, ProtocolDefinition, (entities) => this._callback(entities), log);
    ...
    switch (name) {
      ...
      case 'Piot.Basal.Quaternion':
        return [
          ...
          {name: 'c', type: 'int'}, {name: 'mode', type: 'int'}
        ];
  ...
  _callback(entities) {
      ...
      for (let componentFieldDefinition of typeDefinition.definition.fields) {
        ...
        let componentTypeDefinition =
            ProtocolDefinition.definition
                .components[componentTypeDefinitionName];
    ...
    const root = {entities: entitiesDto};
